...
// =============================================================================
// /school/coach — Principal AI Coach
// -----------------------------------------------------------------------------
// Chat-style page that lets a super_teacher ask questions about their school's
// quiz performance. Posts to /api/school/coach with the bearer token; appends
// each reply to a local history. Renders a typing-dots state while waiting and
// shows suggested-question chips when there's no history yet.
// =============================================================================
...
const SUGGESTIONS = [
  "Why did our average drop?",
  "Which class needs attention?",
  "Who's our top improver?",
  "Suggest a faculty meeting topic.",
];
...
export default function PrincipalCoachPage() {
  ...
  async function sendMessage(messageOverride?: string) {
    ...
    try {
      ...
      const res = await fetch("/api/school/coach", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          message,
          // Only ship the prior turns (not the message we just added) so the
          // server's transcript matches what the model expects.
    ...
    <div className="max-w-4xl mx-auto fade-in flex flex-col" style={{ minHeight: "calc(100vh - 4rem)" }}>
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div className="min-w-0">
          <Link href="/school" className="muted text-xs inline-flex items-center gap-1 hover:underline">
      ...
      <div className="mt-4 sticky bottom-0">
        ...
        <p className="muted text-xs mt-3 text-center">
          Coach answers are based on data as of {snapshot ? relativeFromNow(snapshot.asOf) : "your last question"}.
...
function EmptyState({
  ...
  return (
    <div className="card text-center py-10">
      ...
      <h2 className="text-lg font-semibold">Your data-savvy second-in-command</h2>
      <p className="muted text-sm mt-1 max-w-md mx-auto">
        I&rsquo;ve already pulled this week&rsquo;s scores, top students, and
        at-risk signals. Ask anything &mdash; I&rsquo;ll cite the numbers.
      </p>
