...
// =============================================================================
// /school/digest — Weekly Principal's Brief
// -----------------------------------------------------------------------------
// One-shot view. On load it POSTs to /api/school/digest, which returns a
...
export default function PrincipalDigestPage() {
  ...
  async function fetchDigest(opts: { isRefresh?: boolean } = {}) {
    ...
    try {
      ...
      const res = await fetch("/api/school/digest", {
        method: "POST",
        headers: {
  ...
  return (
    <div className="space-y-6 mt-6">
      ...
      <p className="muted text-xs text-center pt-2">
        Generated by AI from your school&rsquo;s last 30 days of data. Always verify before acting.
      </p>
    </div>
  );
}
...
function OwnerChip({ owner }: { owner: "principal" | "teacher" | "both" }) {
  const map = {
  ...
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold flex-shrink-0 mt-0.5 ${map[owner]}`}>
      {label}
    </span>
  );
}
