...
import { sendEmail, coTeacherInviteTemplate } from "@/lib/email";
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Send the invite email. Best-effort — the row is the source of
    // truth (the recipient also sees the invite in their dashboard);
    // email is the courtesy nudge so they don't have to log in to know.
    // Vipin caught this on 2026-05-12: co-teachers were never receiving
    // a notification, so invites silently piled up. See lib/email.ts.
    ...
    try {
      ...
      const tmpl = coTeacherInviteTemplate({
